...
def check_source_types(rep, path, fm):
    ...
    for s in fm["sources"]:
        ...
        # **未確認を素通りさせない。**
        # 以前は「未確認は確認状態だから種別で上書きしない」として例外にしていたが、
        # そのせいで判定表が知っている出典まで未確認のまま紙面に残った(実測7件)。
        # 未確認でよいのは、判定表がそのドメイン・アカウントを知らないときだけで、
        # そのときは classify_source も未確認を返すので、下の照合が通る(規程2.5)
