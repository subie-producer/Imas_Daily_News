...
- ただし **Grok は週次のセッション上限がある**(SuperGrok)。1回の収集で10セッション(=ブランド10面)を消費し、日5回だと週350セッションで上限の約3倍になる(2026-08-24 実測: 66セッションで週上限の56%)。**絞るのはブランド数ではなく実行回数**とし、`.env` の `GROK_HOURS`(JST の時。既定 `03,18`)で Grok を回す時刻だけを限定する。定点観測と Claude 探索(haiku)は安価なので毎回そのまま回す。締切前スイープの 03 は compose の直前なので外さないこと。
