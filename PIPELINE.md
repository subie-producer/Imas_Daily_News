...
| # | クエリ | 狙い |
|---|--------|------|
| 1〜6 | ブランド別(シャニ/学マス/シンデレラ/ミリオン/765AS/SideM。ヴイアラ・その他は隔日) | 公式告知・ガシャ・グッズ・**締切**(続報キューの種) |
| 7 | 全体トレンド(72時間) | 全国トレンド入り・バズ(「アイマス婚」級の現象) |
| 8 | ファン文化 | 高エンゲージのファンアート・コスプレ・ノスタルジア(声優逸話・周年ネタ)。ファン種別記事とランキングのトレンド分の種 |

- 各クエリ最大8件・エンゲージメント(高/中/低)と投稿日時を必須項目にする。合計目安 30〜50件/回。
...
- **標準 10〜14本/下限 8本/上限なし**(あふれは全て small に吸収)。
- **lead 欠落はブロック**(一面のない新聞はあり得ない。lint エラー)。
- **8本割れは警告+Discord 通知の上で発行(GO)**。下限をブロックにすると発行不能=休刊に倒れ、日刊の信頼を損なう方が痛い。
