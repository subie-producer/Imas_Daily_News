...
- **compose**(04:00): `scripts/compose.py` が四段で実行する(執筆時コンタミの構造的排除):
  ...
  2a. **執筆は構造化出力**(`WRITE_MODE=structured`、既定): 執筆セッション(Codex)は `codex exec --output-schema`(`schema/article-out.schema.json`)で、見出し・リード・段落(**段落ごとに根拠の事実 id F1…**)・tags・出典(url と label だけ)・event_date(1日だけ)か、abort(理由コード)を JSON で返す。ファイルは書かない。`scripts/renderlib.py` が事実 id・出典・タグ・日付を検算し、frontmatter(slug/brand/candidate_ids は計画、種別は判定表、src は最弱、rank は仮、訂正欄は固定値)を作ってファイルにする。**構造は生成時に強制する**ので、事後の正規化や「lint 赤→修正セッション」は要らない(lint は最後の不変条件の検査。赤ならバグ)。実測: 1本 0.5分。検算の中身(監査で固めた): 見出し・リード・全段落に根拠 id / 素材に無い URL は `new_facts`(N1…: 執筆が一次情報を読んで確かめた事実と読んだ URL)に書いたものだけ / tags 2〜4 / event_date は素材か new_facts に出てくる日 / abort は理由コードと説明が必須 / roundup・culture は3件以上の素材を使う。素材の `unbacked_facts` は落とさず渡す(確かめられれば new_facts に書ける)。段落の根拠 id は本文末尾に `<!-- F1 N2 -->` で残し(字数には数えない)、new_facts は frontmatter の `verified_facts` に残す(校閲が段落と根拠を突き合わせるため)。校閲のブロックで書き直すとき(`revise_articles`)は指摘に `issue_id`(I1…)を振り、対応したと言う id が渡したものの中にあること・その引用が新しい稿に残っていないことを機械で検める(直したふりを通さない)。`event_date` の正規化は文字列中の**いちばん早い**日付(範囲は開始日。令和も読む。暦に無い日は捨てる)
