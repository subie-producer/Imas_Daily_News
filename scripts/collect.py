...
# ただし**下げすぎると1件も書けない**: 3ターンでは 9面すべてが0件で終わった
# (1ターン目が段取り、2ターン目が検索、書き出しに到達しない)。
# 検索65回・入力1.56M を消費して収穫ゼロ。**ターン数は打ち切りの安全弁であって
# 検索予算の制御には向かない。**掘り方(どこで見切るか)はプロンプトで導く。
GROK_MAX_TURNS = int(ENV.get("GROK_MAX_TURNS", "8"))
