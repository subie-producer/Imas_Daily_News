...
def slugify(brand: str, key: str, taken: set[str]) -> str:
    import hashlib
    # 区切り(記号・空白・既存ハイフン)はすべて 1 本のハイフンに畳む。既存ハイフンを残すと
    # 「ハイフンで挟まれた日本語」が畳まれても両隣のハイフンが残り、slug が `ex-----28` のように
    # 連続ハイフンになって英小文字ハイフン形式の検証に落ちる(2026-09-16 の計画不成立)
