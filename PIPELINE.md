...
- **compose**(03:00。2026-09-18 から。06:00 の発行までに、止まったら当番(Opus 修正 → Sol 監査)が直して再実行できる時間を残す): `scripts/compose.py` が四段で実行する(執筆時コンタミの構造的排除):
  ...
  2a. **執筆は構造化出力**(`WRITE_MODE=structured`、既定): 執筆セッション(Codex)は `codex exec --output-schema`(`schema/article-out.schema.json`)で、見出し・リード・段落(**段落ごとに根拠の事実 id F1…**)・tags・出典(url と label だけ)・event_date(1日だけ)か、見送り(status=decline。理由コード付き)を JSON で返す。ファイルは書かない。`scripts/renderlib.py` が事実 id・出典・タグ・日付を検算し、frontmatter(slug/brand/candidate_ids は計画、種別は判定表、src は最弱、rank は仮、訂正欄は固定値)を作ってファイルにする。**構造は生成時に強制する**ので、事後の正規化や「lint 赤→修正セッション」は要らない(lint は最後の不変条件の検査。赤ならバグ)。実測: 1本 0.5分。検算は**形だけ**: 見出し・リード・全段落に根拠 id が付いていて実在する / 出典 URL は素材か `new_facts`(N1…: 執筆が一次情報を読んで確かめた事実と読んだ URL)のもの / tags 2〜4 / event_date は日付の形 / 見送り(decline)は理由コードと説明が必須 / roundup は3件以上の素材を使う(ファン面 culture は素材が1件でも載せる。編集長の決定 2026-09-18)/ 1 block = 1 段落 / HTML・参照リンク・不可視文字・制御文字を書かない。**中身の判断(出典を隠していないか、new_facts を本当に読んだか、event_date が素材と合うか、書き直しで残した記述が正しいか)は校閲(モデル)の項目で、機械ではしない。****検算に落ちても記事は落とさない**: 落ちた点だけを示して同じ契約で1回だけ書き直させる(差し戻し)。それでも通らなければ理由を記録して外し、Discord に理由ごと知らせる(検算が厳しすぎるのか執筆が守れないのかを人が見る)。執筆の結果は「見送り(執筆の判断)/ 差し戻し(検算不合格の1回のやり直し)/ 落とした(2回目も不合格)」の3つで、一括りに「不成立」と呼ばない。素材の `unbacked_facts` は落とさず渡す(確かめられれば new_facts に書ける)。段落の根拠 id は本文末尾に `<!-- F1 N2 -->` で残し(字数には数えない)、new_facts は frontmatter の `verified_facts` に残す(校閲が段落と根拠を突き合わせるため)。校閲のブロックで書き直すとき(`revise_articles`)は指摘に `issue_id`(I1…)を振り、対応したと言う id が渡したものの中にあること・その引用が新しい稿に残っていないことを機械で検める(直したふりを通さない)。`event_date` の正規化は文字列中の**いちばん早い**日付(範囲は開始日。令和も読む。暦に無い日は捨てる)
  2b. **取りこぼしの拾い直し**: 面別の選定で articles にも dropped にも現れなかった主題は、通知して終わりにしない。**面は決めつけない**。主題は収集時の brand で面に配られるが、担当が「自分の面の話ではない」と思ったとき、不採用の語彙に「面違い」が無いので黙って落とすのがいちばんありそうな経路だからである。面の役割を持たない1セッションに、消えた主題と**全面の既存記事**を見せ(`replan_missing`)、主題ごとに「**どの面の話か** → 記事化 / 既存記事・roundup へ統合(面をまたいでよい) / 理由付き不採用」を答えさせて計画へ機械で反映する。それでも残ったものだけを通知する(実測 2026-09-06: cg 面に配られた公式ポスト1件が無言で消えていた)
  3. **社説(2026-09-06 号で終了。以後は書かない。`pipelib.EDITORIAL_UNTIL`)**: 専任のコラムニストセッション(紙面名義「AI疑似プロデューサー」)が当日の記事群だけを事実源として1本書く。人格規程: 一人称の個人コラム・偏愛と断言・ユーモアの矛先は自分と状況のみ(批判封印)・紙面要約の禁止。「プレイできない・会場に行けない書き手」という構造的な立場は自覚してよいが、AIであることは名乗らない(名義は編集部の決めごとで本人は無自覚)。**人格の二層構造**: 根幹=`prompts/columnist-core.md`(不変の憲法。全マスの equal dignity・愛の定義・接続者/翻訳者/記録者/伴走者・事実/解釈/願望の区分け。人間のみ改訂可、プロンプトに全文埋め込み)+個性=`stock/columnist.md`(手帳。執筆前に読み、執筆後に日誌14日分・偏愛/持論20行・追跡中の物語10件を上限に自己編集。根幹に反する方向へは育てない)。手帳は事実源にはしない
  4. **組版**(`scripts/assemble.py`): コードが記事の見出し・リード・面・日付付きの事実を圧縮して渡し、**1つの構造化セッションが判断だけを JSON で返す**(digest の12行 / 既報として残す事実 / 続報予約 / pending。`schema/assemble.schema.json`)。反映はコード(号スナップショット・stories・scheduled・pending)で、**冪等**。この号の寄与に号の印を付け、再実行時は剥がしてから付け直すので、記事が落ちても増えてももう一度呼ぶだけ。校閲が既報判定に使う組版前の台帳(`metrics/stories-before-<日付>.yml`)もここで書く。`derive.py` と lint はコードが回し、赤なら組版の欠陥として人へ渡す(モデルに直させない)。以前は Claude の1セッションに丸ごとやらせて40本で25分・ツール呼び出し96回だった(2026-09-11 実測)。今は8分
  4b. **記事の固定項目はコードで正規化**(`canonicalize_article`): slug/edition/brand/candidate_ids は計画から、出典種別は判定表から、src は最弱、label が無ければ候補の題名。執筆セッションに写させない・直させない
  4b'. **紙面の出典で判定表に無いものは、発行前に決める**(`pipelib.classify_retag_lint(date, posts_only=True)`。執筆の直後・組版の前): 執筆が自分で見つけた URL は候補に無いので、収集の合議では判定されない。その号の記事の未確認の出典だけを対象に、登録済みチャンネルの動画 ID は機械で、それ以外は合議で判定表に足し、記事の種別を付け直す(収集と同じ取引: 判定表 → 付け直し → lint、失敗したら戻す)。以前は翌朝の watch が「未確認の出典」を鳴らし、昼の収集で直っていた(2026-09-19: 公式チャンネルの動画3件)。締切の余裕が無ければ飛ばす(未確認のまま出て、翌朝の watch に出る)
  4c. **approve は中身の指紋に結び付く**: 校閲が見る部分(本文・見出し・リード・出典・面・素材 id、digest と一面)の sha256 を review JSON に残し、release は現在の指紋と一致する approve だけを通す(違えば stale)
  4d. **lint は構造の不変条件だけを赤にする**。時制矛盾・発行前の時刻のような語の近接で見るヒューリスティックは `[所見]` の警告にして、その記事の校閲プロンプトに添える。校閲が文脈を確かめて判定する(検査の所有者と最終判定者を分ける)
  4e. **当番(`scripts/oncall.py`)**: 工程が「人間判断が必要」で止まったら、人へ投げる前に当番が動く。当番(Opus)が journal と状態から診断し、main から切った使い捨ての作業ツリーで最小の修正を書き、selfcheck と再現テストを通す → 監査(Sol)が差分を敵対的にレビュー → **当番と監査の仕事の範囲は「この号を発行するのに必要な最小限で、今後もちゃんと動く正しい修正」**(編集長の整理 2026-09-18)。監査の指摘は2つに分ける: **must_fix**=この範囲の問題(止まった原因に当たっていない/現実の入力でまた止まる/データや他の工程を壊す/その場しのぎ)、**later**=発行後でよいもの(いまは止まらないが現実に起きる見込みのあるもの: 堅牢化・設計の改善・テスト・書き方)。**起きる道筋を言えない指摘は、指摘として扱わない**(編集長:「起きねーよ。どういうときに起きると言えるんだ、どのくらいの確率で起きる見込みなんだ、で弾いていい」): 監査はどの指摘にも `occurs`=どういうときに起きるか(実データ・ログ・運用から到達する道筋)と、どのくらい起きそうか、を書く。「schema 上は可能」は道筋ではなく、書けないものは must_fix にも later にも入れない。当番は道筋の無い指摘を実データの根拠を示して弾いてよく、`occurs` が空の later はコードでも保管しない。must_fix は**合意(approve で must_fix が空)するまで**当番が直し、監査が見直す(上限は `MAX_ROUNDS`=6 往復と `ONCALL_LIMIT_MIN`=150 分)。later は当番がその場では直さず、取り込みのあと `metrics/oncall-backlog.jsonl` に保管して修正報告に載せ、watch が毎朝「未着手 N件」を出す。発行して落ち着いてから直し、`oncall.py --backlog-done <key>` で消し込む(保管の失敗は発行を妨げない)。以前は範囲を決めずに合意を求めたので、監査が「現実には来ない入力での堅牢化」を must_fix に積み、発行できる修正が2往復で時間切れになって号が止まった。「当たらない」と答えてよいのは、指摘が事実誤認か、起きる道筋が無いか、編集方針で判定対象外のもの(`POLICY_EXCLUDED`: 内容判断の機械化の要求、HOME/OS 隔離の要求)のときだけ。差分が無い(no_fix_needed)ときも監査は通す(誤診を検める)。合意したもの(approve、かつ must_fix が空。later は残っていてよい)だけ、**その commit ハッシュ**を main に merge して push し、生きている edition ブランチへ main を取り込み、止まった工程を再実行する。再実行は直した箇所を踏ませる: 計画・執筆・組版の層(compose.py / planlib / renderlib / assemble / prompts / 出力 schema)を直したか当番が rebuild と答えたなら、edition ブランチの控え(`backup/<日付>-<時刻>`)を push してから、stock からこの号の寄与を剥がし(`assemble.rollback`: 台帳は edition_facts の印、予約は reserved_on、pending は組版前の控えへ。控え `metrics/*-before-<日付>.yml` は消さない)、記事・号スナップショット・計画・校閲記録を外して **compose を最初から**。失敗したら控えへ戻す(force-with-lease)。それ以外は古い校閲記録を消して `--reuse-plan`。当番が none と答えたら(同じ入力では同じ結果)人へ。上限までに直し切れなければ、そこまでの修正(commit。記録ブランチ `repair-wip/…`)と残った指摘を state(`wip`)に残して人へ。**次の試行はその続きから**始める(基準の main が同じなら。以前は毎回ゼロから診断し直し、2026-09-18 は1回目と2回目が別々の仮説を立ててどちらも途中で終わった)。**修正を入れたら Discord に報告する(必須)**: 診断・原因・検証・監査の判定・差分の要点・戻し方(main への取り込みは `--no-ff` で merge commit を作り、`git revert -m 1 <merge commit>` を報告する)。全文は `metrics/oncall-<日付>-<工程>-report.md`。守り: 当番の作業ツリーに `.env` を置かない、触れるのは scripts/ prompts/ schema/ と設計文書だけ(判定表・収集対象は含めない)、セッション後に本体の作業ツリーが汚れていたら取り込まない、Git 操作は全部 fail-closed(記録ブランチは一意な名前で force しない、main の push に失敗したら edition に触らない)、同じ日・工程で2回まで(前提を確かめてから消費)、工程の排他は `pipelib.job_lock`(`metrics/jobs.lock` の flock。collect/compose/release/当番が全部取る。当番は親の compose が離すまで待って取り、再実行が終わるまで持つ。当番が起動する子は `IMAS_JOB_LOCK=held`)、取り込み前の事前通知が Discord に届かなければ取り込まない、予算と時間の上限、当番の状態・記録(`metrics/oncall-*`)は Git 管理外、当番の再実行には `ONCALL=off`(`.env` の `ONCALL=off` で全体を止められる)。当番へ渡す点: 計画が機械検証を通らない・執筆全滅・lint 赤が解消できない・発行前に判断が要る・例外停止(compose)、校閲が approve していない・発行前 lint 赤(release)。
  4i. **申告前になぜなぜ(2026-09-26)**: 人に「異常」として通知する事柄は、申告で終えずに当番が**原因まで遡る**(編集長「すべてのエラーがなぜ起きたかなぜなぜしろ」「申告前に診断しろ」「それを Opus/Sol 系に落とせない設計がカス」)。`pipelib.notify(ok=False)` と `anomaly()` が process の台帳 `ANOMALIES` に積み、`escalate` は毎回その台帳を `prompts/oncall-whywhy.md`(1件ずつ「事象→なぜ→なぜ→原因→対処」。契約・コードの欠陥なら直す、データ・外部・判断の割れならそう診断して材料を書く)で依頼に付ける。工程の終わり(compose の準備完了・人間判断・例外・SIGTERM、collect と release の終了、watch の異常時と例外)に `diagnose_anomalies(stage, 発生日, rerun=False, edition=取り込み先の号)` で当番を呼ぶ(同じ process で既に呼んでいれば重ねない。`--no-rerun`: 号は確定しているので再実行せず、次の実行から効く。試験実行では呼ばない)。工程は compose / release に **classify / collect / watch** を足した(journal は classify→collect)。実例2つ: (1) ニコニコ生放送の URL を「判定の単位を取れない」と人に通知 → `platform_unit` は skip(持ち主が無い)と **parser(持ち主はいるが取る手段が無い=コードの不足)** を分け、parser は `classify_sources.request_parsers` が当番(`classify` 工程、`prompts/oncall-parser.md`)にパーサを書かせる(ホストごと1件、`metrics/parser-requests.json` で3日は出し直さない。当番の再実行は `classify_retag_lint(require_parsers=True)` で、パーサの無い URL が残れば失敗にして記録を消す)。(2) 執筆 schema の出典 label 上限 80 字で告知タイトルが切れ、校閲 R16 が3巡ブロックして記事を落とし、人には「校閲ブロックで落とした」とだけ届いた(第12号: 316 label 中 73 が丁度 80 字) → compose は「書き手が書き直した(fixed/kept)のに同じ記事に同じ規則の指摘が残った」「差し戻しても検算を通らない」を `STUCK` に記録し、外れた記事(`DROP_LOG`)と共に台帳へ。上限は schema と `renderlib.LABEL_MAX` の両方を 200 に
  4f. **時間予算**(`compose.afford` / `terminal_cost`): 締切は 120 分(`COMPOSE_LIMIT_MIN`)、目標 60 分は報告だけ。判断の単位は**最後のサイクル**=「ブロック記事を落とす → 社説∥組版のやり直し → lint → 校閲1波 → commit/push」で、その所要は実測(組版×1.2 + 校閲1波×1.2 + 3分。無ければ組版9分の保守値)。校閲の往復をもう1巡始めるのは、その巡+書き直し+取り直しのあとに**最後のサイクルが丸ごと残る**ときだけ。最後のサイクル自体は、その分だけ残っていれば始める(2026-09-17: ここに更に余白を上乗せしていたため、残り17分で落とせる2本を落とさず号ごと止まった)
  4g0. **依頼文(プロンプト)は `prompts/` に置き、指示だけを書く**(`pipelib.render_prompt`。台帳は `PROMPTS.md`): コードは `{大文字}` の差し込み口に値を埋めるだけで、依頼文の本文をコードに埋めない。構成は MECE(役割と仕事 → 手順 → 判断の規則(同じ規則を2か所に書かない)→ 出力 → 入力・素材は最後)。コードや schema が決めることを繰り返さず、使っていない経路の指示を残さない。**規則の理由・経緯・事故の記録は依頼文ではなくデータ**なので `PROMPTS.md` の 3 章に書く(編集長 2026-09-18)。`test_prompts_are_instructions_only` が、経緯の混入・90 行超え・差し込みの取り違え・コードへの埋め戻しを検める。執筆の file モード(執筆セッションが Markdown を書く)は依頼文ごと廃止し、`WRITE_MODE` は structured だけ
  4g. **モデルに渡すファイルの形**(`pipelib.prompt_file` / `claude_traced`): Read は1回に 2000 行・1行 2000 字までしか返さず、超えた分は黙って切れる。(1) **指示が先、素材が後**(素材は号によって伸びる。後ろに置いた指示は読み切れない日が来る)。(2) 値は1行に畳んで埋め、行数が件数だけで決まるようにする(組版は `assemble.render_input` / `_v`)。(3) 行数が上限に迫るファイルは、呼び出しの指示文で行数と「offset を進めて最後まで読む」ことを伝える。(4) 「ファイルを読むな」のような、ファイル渡しと矛盾する文言を書かない。組版セッションは stream-json の経過を `metrics/work/<日付>/assemble-trace-<n>.jsonl` に残し、時間切れのときは「何ターン・どの道具を何に使ったか」を付けて上げる。時間が残っていれば1回だけやり直す(判断は入力の純関数、反映は冪等)。2026-09-18: 組版の指示ファイルが初めて 2000 行を超えた号で、末尾の指示が1回の Read に入らず 900 秒で時間切れになり、経過が残っていなかったので当番も監査も原因を推測するしかなかった
  4h. **判定表を書く入口は1つ**(`pipelib.write_source_table`): 読むときと同じ検査に通った本文だけを置き換えで書き、手元の読み込み結果を捨てる。`source_type_table()` はファイルが変わっていれば読み直す。合議の追加は節の中だけを探して差し込み、既にあるものは足さない(`classify_sources.insert_labeled`)。2026-09-18: 古い読み込み結果のまま同じ動画 ID を2回足して表が二重定義になり、付け直しが落ちた。取引失敗の通知には子プロセスの言い分(`collect._err_tail`)を載せる
- **校閲**(04:30目安): **記事1本につき1セッション**を `COMPOSE_WAVE` 本ずつ並列で走らせ、判定を JSON(`--json-schema`)で受け取って1つにまとめる。チェックリストは記事=`prompts/review-article.md` / 社説=`prompts/review-editorial.md` / 紙面全体(主題の重複・記事の漏れ)=`prompts/review-paper.md`。往復と判定結果は metrics に記録する。
  ...
  - 指摘には `rule_id`(項目番号)・`repair`(rewrite_claim / drop_claim / add_source / drop_source / drop_article / none)・`fact_ids`・`expected` を付ける。**修正は執筆側(Codex)が同じ出力契約で書き直す**(`revise_articles`。校閲側は記事を編集しない。別ベンダー校閲の独立を守る)。直した稿は初稿と同じ検算を通してからファイルにし、abort なら落とす。実測: 1本 0.3分
  - lint の `[所見]`(時制矛盾・発行前の時刻)は、その記事の校閲プロンプトに添えられる。校閲が文脈を確かめ、当たっていればブロックにする
