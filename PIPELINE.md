...
- **compose**(04:00): `scripts/compose.py` が四段で実行する(執筆時コンタミの構造的排除):
  ...
  4e. **当番(`scripts/oncall.py`)**: 工程が「人間判断が必要」で止まったら、人へ投げる前に当番が動く。当番(Opus)が journal と状態から診断し、main から切った使い捨ての作業ツリーで最小の修正を書き、selfcheck と再現テストを通す → 監査(Sol)が差分を敵対的にレビュー → 当番が指摘を敵対的に取り込む(受け入れるものは直し、当たらないものは根拠で反論)。往復は最大2回。合意したものだけ main と生きている edition ブランチへ入れ、止まった工程を再実行する(compose は `--reuse-plan`)。合意できなければ両者の言い分を添えて人へ。守り: 触れるのは scripts/ prompts/ schema/ と設計文書だけ、同じ日・工程で2回まで、走行中の compose/release があれば待機、予算と時間の上限、当番の再実行には `ONCALL=off`(`.env` の `ONCALL=off` で全体を止められる)
