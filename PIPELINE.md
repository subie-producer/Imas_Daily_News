...
- **compose**(04:00): `scripts/compose.py` が四段で実行する(執筆時コンタミの構造的排除):
  ...
  2b. **取りこぼしの拾い直し**: 面別の選定で articles にも dropped にも現れなかった主題は、通知して終わりにしない。**その主題だけ**を同じ面の担当に突き返し(`replan_missing`)、記事化 / 既存記事・roundup へ統合 / 理由付き不採用のどれかを答えさせて計画へ機械で反映する。それでも残ったものだけを通知する(実測 2026-09-06: cg 面の公式ポスト1件が無言で消えていた)
